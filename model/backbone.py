...
class DinoVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.sin_cos_embeddings:
            ...
            logger.info(
                "using multiple position embeddings (one for global one for local)"
            )
            self.pos_embeds = nn.ParameterDict()
            self.pos_embeds[str(img_size)] = nn.Parameter(
                torch.zeros(1, num_patches, embed_dim)
            )
            n_local_patches = (local_crops_size // patch_size) ** 2
            self.pos_embeds[str(local_crops_size)] = nn.Parameter(
                torch.zeros(1, n_local_patches, embed_dim)
            )
            self.pos_embed = None
        else:
        ...
        self.head = (
            nn.Linear(final_chs, num_classes) if num_classes > 0 else nn.Identity()
        )
    ...
    def reset_classifier(
        self, num_classes: int, global_pool=None, representation_size=None
    ):
        self.num_classes = num_classes
    ...
    def mask_patches_with_probability_p(self, x, mask_ratio_tuple, p):
        ...
        masks = torch.stack(
            [
                torch.BoolTensor(
                    self.mask_generator(
                        int(N * random.uniform(mask_ratio_min, mask_ratio_max))
                    )
                )
                for _ in range(0, n_samples_masked)
            ]
            + [
                torch.BoolTensor(self.mask_generator(0))
    ...
    def prepare_tokens(
        ...
        if do_ibot:
            if ibot_balanced_masking:
                ...
                x, masks, n_masked_patches_upperbound = (
                    self.mask_patches_with_probability_p_upperbound(
            ...
            cls_token = (
                cls_token + 0 * self.mask_token
            )  # hack to use the mask_token param to not crash ddp...
    ...
    def forward_features(
        ...
        x, masks, n_masked_patches_upperbound = self.prepare_tokens(
            x, mask_ratio_tuple, mask_sample_probability, ibot_balanced_masking
        )
